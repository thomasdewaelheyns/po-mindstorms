...
(De samenvatting wordt later verder aangevuld met belangrijke aspecten die tijdens het verdere verloop van het project naar voor treden.)
...
De doelstelling van de volgende demo is het volgen van een ononderbroken lijn. Deze lijn kan wit of zwart zijn en is uitgezet op bruine panelen.

\section{De robot}
\label{section:lichtsensor}

Voor het volgen van een lijn maken we gebruik van een grijswaardesensor. Deze werd vooraan op de robot bevestigd.

Tijdens het onderzoek voor de verschillende onderdelen van demo 2, is het ontwerp van de robot in zijn geheel ge"evolueerd. De belangrijkste aanpassing is het omkeren van de rijrichting, waarbij het zwenkwiel vooraan komt te staan en we dus een achterwiel aangedreven robot bekomen.

De belangrijkste reden voor deze aanpassing, was een probleem bij het oprijden van een helling. Met de nieuwe opbouw kan de grijswaardesensor dichter bij de as geplaatst worden, waardoor deze zich sneller opnieuw in een juiste hoek tov. de ondergrond zal bevinden. Ook de NXT brick werd in een horizontale positie geplaatst, waardoor montage van alle sensoren en motoren eenvoudiger werd.

\section{De software}

Voor deze demo diende software ontwikkeld te worden die de robot in staat stelt om een lijn autonoom te volgen. De meetresultaten van de grijswaardesensor dienen tevens verzonden te worden via Bluetooth naar een PC, waar ze gevisualiseerd moeten worden. De ontwikkeling voor robot en PC worden in dit deel verder toegelicht. De communicatie component wordt in bijlage \ref{appendix:bluetooth} in detail besproken.

\subsection{Robot}

De grijswaardesensor kan gekalibreerd worden, waarna deze genormaliseerde waarden terug geeft van 0 tot 1024\footnote{Re\"ele waarden liggen typisch tussen 145 voor een meting in een donkere omgeving en 890 in klaar zonlicht.}. Voor het kalibreren kunnen een donker en licht waarde aan de Lejos API gegeven worden, welke dan gebruikt worden om de genormaliseerde waarden op af te stemmen.

Het algoritme van de robot is zeer eenvoudig gehouden, om de betrouwbaarheid te bevorderen. De volgende stappen worden in een oneindige lust uitgevoerd:
...
\item indien niet (meer) op een lijn, zoek een lijn door ter plekke te draaien in steeds groeiende hoeken tot een lijn gevonden is
\end{enumerate}

Voor dit algoritme werd de manier waarop de Lejos API aangesproken werd ook gewijzigd. Voor het rijden van veelhoeken werd aan de Lejos API de opdracht gegeven om een bepaald aantal omwentelingen te doen. Vervolgens werd er gewacht om een synchrone verwerking te simuleren voor onze software. Met de introductie van sensoren en de nood om acties te ondernemen voor een bepaalde afstand bereikt is, is deze synchrone verwerking verwijderd en zal de controle onmiddellijk teruggegeven worden aan de controleren applicatie, in dit geval het lijnvolger algoritme.
...
We hebben met verschillende sets van kalibratiewaarden telkens 12 metingen gedaan van bruin, wit en zwart. Voor elk van deze meetwaarden hebben we vervolgens het gemiddelde berekend en de relatieve afwijking van elk van de gemeten waarden. Figuren \ref{chart:bruin}, \ref{chart:wit} en \ref{chart:zwart} geven respectievelijk de resultaten voor bruin, wit en zwart weer.

\begin{figure}
	\caption{Relatieve afwijking tov. gemiddelde gemeten grijswaarde voor bruin}
	\label{chart:bruin}
	\vspace{10pt}	
	\centering
	...
	\tikzset{external/remake next}
	\begin{tikzpicture}
		\begin{axis}[xmin=0,xmax=4,ymin=-8, ymax=8,
			xtick={1,2,3,4},xticklabels={1,2,3},
	      		xlabel={Kalibratie set},
			ylabel={Relatieve afwijking tov verwachting (\%)},
			]
			%#1: center, #2: median, #3: 1/4 quartile, #4: 3/4 quartile, #5: min, #6: max
			\boxplot{1}{-0.34}{-1.68}{2.36}{-7.07}{5.05}
			\boxplot{2}{-1.68}{-3.03}{2.69}{-7.07}{7.74}
			\boxplot{3}{-0.11}{-1.44}{1.55}{-6.77}{5.22}
		\end{axis}
	\end{tikzpicture}
\end{figure}

\begin{figure}
	\caption{Relatieve afwijking tov. gemiddelde gemeten grijswaarde voor wit}
	\label{chart:wit}
	\vspace{10pt}	
	\centering
	...
	\tikzset{external/remake next}
	\begin{tikzpicture}
		\begin{axis}[xmin=0,xmax=4,ymin=-16, ymax=20,
			xtick={1,2,3,4},xticklabels={1,2,3},
	      		xlabel={Kalibratie set},
			ylabel={Relatieve afwijking tov verwachting (\%)},
			]
			%#1: center, #2: median, #3: 1/4 quartile, #4: 3/4 quartile, #5: min, #6: max
			\boxplot{1}{-0.49}{-3.66}{2.44}{-10.24}{16.10}
			\boxplot{2}{-0.66}{-3.38}{3.29}{-9.06}{8.73}
			\boxplot{3}{1.82}{-9.85}{4.55}{-15.56}{19.21}
		\end{axis}
	\end{tikzpicture}
\end{figure}

\begin{figure}
	\caption{Relatieve afwijking tov. gemiddelde gemeten grijswaarde voor zwart}
	\label{chart:zwart}
	\vspace{10pt}	
	\centering
	...
	\tikzset{external/remake next}
	\begin{tikzpicture}
		\begin{axis}[xmin=0,xmax=4,ymin=-600, ymax=490,
			xtick={1,2,3,4},xticklabels={1,2,3},
	      		xlabel={Kalibratie set},
			ylabel={Relatieve afwijking tov verwachting (\%)},
			]
			%#1: center, #2: median, #3: 1/4 quartile, #4: 3/4 quartile, #5: min, #6: max
			\boxplot{1}{68}{-256}{200}{-580}{476}
			\boxplot{2}{-26.15}{-90.77}{38.46}{-210.77}{269.23}
			\boxplot{3}{0}{-100}{78.57}{-228.57}{257.14}
		\end{axis}
	\end{tikzpicture}
\end{figure}

De resultaten tonen aan dat met verschillende kalibraties, de resultaten voor elk van de kleuren binnen vergelijkbare bereiken liggen. Voor zwart dient opgemerkt te worden dat hier in verhouding tot bruin en wit, veel grotere relatieve afwijkingen optreden. We kunnen dit toeschrijven aan het vuil worden of slijten, waaraan zwart het meest onderhevig is.

Voor het bepalen van de waarden waarbinnen we besluiten of een kleur bruin, wit of zwart is, bekijken we de resultaten in absolute vorm over alle metingen heen. Figuur \ref{chart:kleuren} illustreert de bereiken van de drie verschillende kleuren. Hierbij merken we op, dat ofschoon zwart een grotere potentiële relatieve afwijking kent, dit in praktijk geen probleem zal opleveren. Het bereik van de metingen voor zwart ligt ver van het bereik van bruin en wit. Deze twee laatsten daarentegen hebben wel minima en maxima die in elkaars buurt liggen en potentieel kunnen zorgen voor "valse positieven" in beide richtingen. 
...
De doelstelling van de volgende demo is het volgen van een ononderbroken muur.
...
Dankzij de richtbare sonar, kan de robot zowel de afstand tot de te volgen muur meten als voor zich controleren of er geen muur, ten gevolge van een hoek in het parcours, volgt. De robot zal de sonar dus constant heen en weer laten bewegen om deze twee meetpunten te controleren.
...
In een eerste test bepaalden we de absolute afwijking op een afstandsmeting van 1 tot 60cm. Figuur \ref{chart:sonarDistance} geeft de relatieve fout weer op de gemeten afstanden.

\begin{figure}[htbp]
   \centering
   \caption{Test 1: Relatieve afwijking van de sonarsensor tov de te meten afstand.}
\begin{tikzpicture}    \begin{axis}[        xlabel=Werkelijke Afstand (cm),        ylabel=Relatieve fout tov gewachte afstand (\%)]    \addplot[color=red,mark=x] coordinates {
	(5,40.00)
	(10,30.00)
	(20,15.00)
	(40,2.50)
...
Een robot zal niet steeds loodrecht staan tov. een obstakel. Om na te gaan hoeveel de meetresultaten afwijken onder een hoek, hebben we een test gedaan waarbij we onder een hoek van 0, 5, 10, 15, 30 en 45 graden de gemeten afstand vergeleken met de werkelijke afstand. Figuur \ref{chart:sonarAngle} geeft de relatieve afwijking van de meting tov de werkelijkheid weer.

\begin{figure}[htbp]
   \centering
   \caption{Test 2: Relatieve afwijking van de sonarsensor bij meting onder een hoek.}
\begin{tikzpicture}    \begin{axis}[        xlabel=Werkelijke Afstand (cm),        ylabel=Relatieve fout tov gewachte afstand (\%),
...
De robot vertrok op 180cm van het obstakel. Zoals we reeds opmerkten in de voorgaande testen, is de sonar relatief nauwkeurig op grotere afstanden. Vanaf 30cm vergroot de fout aanzienlijk. Dat de fout vlak bij nul tot boven de 100\% gaat heeft hier natuurlijk te maken met de robot die tegen het obstakel rijdt.

Maar er is een tweede piek die zich voordoet rond de 140 tot 130cm. Op deze afstand detecteert de sensor geen obstakels. We vermoeden dat er zich hier een eigenfrequentie optreedt en dat er golffronten samenvallen. Aangezien we voor de doelstellingen van de verschillende demo's geen informatie nodig hebben voor afstanden boven de 80cm, is dit fenomeen onbelangrijk voor ons.
...
De doelstelling van de volgende demo is het detecteren van barcodes op een eenvoudig parcours en op basis van de informatie die deze barcodes aanbieden over het parcours, dit zo goed mogelijk rijden.

\section{De robot}

Voor het lezen van barcodes gebruiken we de grijswaardesensor. Deze is reeds op de robot gemonteerd, zoals besproken in sectie \ref{section:lichtsensor}. Ook de basissoftware om grijswaarden te registeren wordt opnieuw gebruikt.

\section{De software}

Het algoritme dat op basis van de gemeten grijswaarden barcodes detecteert doorloopt volgende stappen:

\begin{enumerate}
\item lees de genormaliseerde waarde van de grijswaardesensor
\item indien de ge\"interpreteerde kleur bruin is, doe niets
\item indien de ge\"interpreteerde kleur niet-bruin is, begin detectie van een barcode
	\begin{enumerate}
	\item indien de ge\"interpreteerde kleur niet-bruin is, bewaar deze data
	\item indien de ge\"interpreteerde kleur bruin is, er minder dan drie niet-bruine waarden opgemeten zijn en er vijf bruin-waarden opgemeten zijn, stop dan de barcode detectie
	\end{enumerate}
\item verdeel de opgemeten waardes in 7 gelijke groepen.
\item neem van elke groep het gemiddelde en vertaal de waarde naar wit of zwart
\item vertaal de 7 wit of zwart waarden naar 1 en 0
\item gebruik een tabel om de effectief gedetecteerde waarde om te zetten naar een gecorrigeerde mogelijke waarde, volgens de ge\"integreerde Hemming-code error-correctie
\end{enumerate}
...
Tabel \ref{tab:focus} geeft de focus van elk teamlid tot op heden weer. Tabel \ref{tab:tijdsregistratie} geeft de gepresteerde uren per week per teamlid weer.
...
\hline 
\endhead
Michiel 		& 	(Coordinator) Bluetooth communicatie, calibratie. \\
Florian 		&	RobotAPI, lijnvolger, opvolgen unit testen. \\
Ruben 		&	RobotAPI, veelhoek algoritme, muurvolger. \\
Thomas 		&	RobotAPI, lichtsensor, barcodelezer. \\
Daniel 		&	Testen, barcodelezer \\
Christophe 	&	(Secretaris) Verslag, simulator, model \& navigator. \\
\hline
\label{tab:focus}
\end{longtable}

\begin{longtable}{l r r r r r r}
\caption{Tijdsbesteding per team lid per week} \\ [0.5ex]

%This is the header for the first page of the table...
\hline\hline
 & Michiel & Florian & Ruben & Thomas & Daniel & Christophe \\ [0.5ex]
\hline 
\endfirsthead
...
\hline 
\endhead
totaal    & 54 & 37 & 41 & 38 & 34 & 75 \\
\hline
week 1 & 10 & 10 & 10 & 10 & 10 & 15 \\
week 2 & 9 & 7 & 8 & 7 & 9 & 16 \\
week 3 & 8 & 8 & 8 & 8 & 5 & 12 \\
week 4 & 9 & 7 & 5 & 8 & 5 & 17 \\
week 5\footnote{Week 5 is nog bezig ten tijde van inlevering verslag.} & 18 & 5 & 10 & 5 & 5 & 15 \\
\hline
