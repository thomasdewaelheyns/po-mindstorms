...
Deze functionaliteiten zijn aanwezig in het huidige protocol, dus is het compatibel met onze implementatie.

\subsection{Nadelen}

In het algemeen ondervinden we geen nadelen van de beslissingen die genomen zijn. Aangezien wij voldoende informatie kunnen verzamelen met een kleine subset, zal het volledige protocol slechts een marginale implementatiekost met zich meebrengen. Volgens ons kan het protocol wel sterk vereenvoudigd worden.

De hele join procedure lijkt overbodig. Wanneer een spook laattijdig ``join''t, zal hij gewoon de volgende berichten horen en zijn wereldbeeld beginnen opbouwen. Eventueel kan een commando toegevoegd worden om reeds gestuurde informatie opnieuw te sturen. In een latere versie van het protocol werd het SHOWMAP commando toegevoegd, wat deze functie gedeeltelijk vervult. Dit overlapt gedeeltelijk met het join principe. Bovendien eist de join procedure dat er exact 4 spoken zijn. Indien een spook crasht, of een extra spook op het kanaal komt, zorgt dit voor problemen. Hiervoor werd een 'override' commando toegevoegd, om toch te starten indien er een spook geen join stuurt. De protocolrestrictie voor 4 spoken is volledig onnodig, net als de join procedure. 

In plaats van dynamisch namen uit te wisselen kunnen evengoed vaste namen afgesproken worden, dit zou het protocol eenvoudiger maken.

Voor het discover commando lijkt het beter om een bitfield te gebruiken voor de aanduiding van de muren. Omdat er zeer veel discover commando's gestuurd kunnen worden kan dit de overhead op de communicatie verkleinen, en zelfs het parsen vereenvoudigen. Met eenvoudige bit-wise shift operaties kan getest worden of een tegel een muur heeft of niet. Nadeel is wel dat men moet weten wat een bitfield is.

Het PLAN commando is naar onze mening onvolledig. Het doorsturen van een pad is pas nuttig indien er ook een vorm van election commando's worden gemaakt om iets nuttig te kunnen doen met de PLAN informatie. Zonder informatie met hoe snel de robot zijn pad aflegt kunnen de andere robots zeer weinig doen met de plannen. Hier zal het waarschijnlijk weer nodig zijn het protocol op te lappen met nog een reeks extra commando's.
Voor ons algoritme is dit commando echter volledig overbodig. Onze robot rijdt op enkel informatie van het doolhof, de positie van de andere robots en pacman. De pad informatie zal niet gebruikt worden in onze implementatie.

Nog een algemene opmerking is dat het protocol moeilijk te parsen is door combinatie van strings en numerieke waardes, inconsistensies in de structuur van de commando's, gebruik van verschillende soorten delimiters, ...

Oorspronkelijk was het idee dat het Ghost Protocol letterlijk gebruikt zou worden in het Maze Protocol. Op deze manier zou de implementatie van het Ghost Protocol kunnen herbruikt worden voor het Maze Protocol. Spijtig genoeg zijn er enkele beslissingen in het Maze Protocol gemaakt waardoor het nu niet meer 100\% compatibel is met het Ghost Protocol.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=90mm]{resources/plotIR.pdf}
...
\begin{figure}
\begin{center}
 \includegraphics[width=90mm]{./resources/bloxplotIR.pdf}
...
\begin{center}
 ...
 \includegraphics[width=90mm]{./resources/boxplotSpiegeling.pdf}
...
Aangezien de kleinste eenheid van een parcours veranderd is van paneel naar sector, hebben we met behulp van refactoring de simulator aangepast zodat een parcours zowel met panelen als met sectoren kan worden voorgesteld. Hierdoor is het mogelijk zowel het project van het eerste semester als het tweede semester uit te voeren. Deze techniek werd verder toegepast op de rest van het herstructureringsproces. 
