...
public class Board extends JPanel {
  ...
  private void renderTile(Graphics2D g2d, Tile tile, int left, int top) {
    ...
    g2d.fill(new Rectangle(TILE_SIZE * (left - 1), TILE_SIZE * (top - 1),
            TILE_SIZE, TILE_SIZE));
    this.renderLinesCross(g2d, tile, left, top);
  ...
  private void renderLinesCross(Graphics2D g2d, Panel tile, int left, int top) {
    ...
    Rectangle horizontal = new Rectangle(left * TILE_SIZE + TILE_SIZE / 2 - LINE_WIDTH / 2,
            top * TILE_SIZE, LINE_WIDTH, TILE_SIZE);
    Rectangle vertical = new Rectangle(left * TILE_SIZE, top * TILE_SIZE + TILE_SIZE / 2 - LINE_WIDTH / 2, TILE_SIZE, LINE_WIDTH);
    g2d.setColor(WHITE);
    ...
    if (tile.hasLine(line)) {
      ...
      switch (line) {
        ...
        case Baring.S:
          ...
          dY = LINE_WIDTH;
          x = offset;
          y = TILE_SIZE - LINE_OFFSET - LINE_WIDTH;
          break;
        case Baring.E:
          dX = LINE_WIDTH;
          dY = length;
          x = TILE_SIZE - LINE_OFFSET - LINE_WIDTH;
          y = offset;
          break;
        case Baring.W:
          dX = LINE_WIDTH;
